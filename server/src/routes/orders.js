...
router.post('/', [
  ...
  body('shipping_address.postal_code').trim().escape().matches(/^\d{5}$/).withMessage('Code postal invalide (5 chiffres)'),
  body('shipping_address.country').trim().escape().notEmpty().withMessage('Pays requis'),
  body('payment_method').isIn(['card', 'paypal']).withMessage('Mode de paiement invalide'),
  handleValidation,
], ctrl.createOrder);
